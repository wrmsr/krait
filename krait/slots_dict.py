# -*- coding: utf-8 -*-
"""
wtimoney@dev20:~/pg/yelp-main$ python slots_dict.py 30 1000000
AdaptiveSlotsDict, z=30, n=1000000... 386,095,488B, 50s
wtimoney@dev20:~/pg/yelp-main$ python slots_dict.py 30 1000000 -d
...
class SlotsClassManager(ClassManager):
    ...
    def __init__(self, class_):
        super(SlotsClassManager, self).__init__(class_)
        self._dict_type = slots_dict.AdaptiveSlotsDict()

    def new_instance(self, state=None):
        obj = super(SlotsClassManager, self).new_instance(state)
...
def SlotsDict(keys, name='SlotsDict'):
    ...
    class SlotsDictBase(collections.MutableMapping):
    ...
    container_type = type(name + '_Container', (object,), {'__slots__': tuple(keys)})
    dict_type = type(name, (SlotsDictBase,), {'__slots__': (), '_container_type': container_type})
    return dict_type
...
def AdaptiveSlotsDict(keys=(), name='AdaptiveSlotsDict'):
    ...
    class AdaptiveSlotsDictBase(collections.MutableMapping):
        __slots__ = ('_container', '_len')
    ...
    def inner(cls):
        ...
        class Wrapper(cls):
            ...
            def __new__(cls, *args, **kwargs):
                obj = super(Wrapper, cls).__new__(cls)
...
def main():
    ...
    def get_mem():
        import pymallocstat
        return pymallocstat.get_status()['status']['vmrss']
    base_mem = get_mem()

    kvs = [('_' + str(i), i) for i in xrange(z)]

    T = dict if opts.use_dict else AdaptiveSlotsDict()
    l = [T(kvs) for _ in xrange(n)]
    ...
    def comma_digit(n):
        s = str(n)[::-1]
        return ','.join(s[i:i + 3] for i in xrange(0, len(s), 3))[::-1]
